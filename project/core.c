...
void coreLoop(void)
{
	...
	do {	// loop until stimulus is complete
		...
		{
			systemTime=systemTime + SCHEDULING_DURATION; // contex switch
			...
			while (releaseEvent != none )
			{
                ...
                // In der Multiprogramming Loop lassen wir Prozesse unblocken mittels unblockProcess Funktion
                // Die unblockiert werden sollen
                unblockProcess();
                if (releaseEvent == started) {
